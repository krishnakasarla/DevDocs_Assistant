...
You can use [transactions](/docs/manual/core/transactions#std-label-transactions) to enforce consistency between collections that contain duplicated data. Transactions update multiple collections in a single atomic operation.
...
Review the different methods to enforce data consistency to ensure that transactions are the best approach for your application. For more information, see [Data Consistency.](/docs/manual/data-modeling/data-consistency#std-label-data-modeling-data-consistency)
...
2. Create the sellers collection
...
3. Configure a transaction to handle updates

   **Note:**

   The following example uses a transaction in [`mongosh`](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh). To see transaction examples for MongoDB drivers, see [Transactions.](/docs/manual/core/transactions#std-label-transactions)
...
- [Enforce Data Consistency with Embedding](/docs/manual/data-modeling/enforce-consistency/embed-data#std-label-enforce-consistency-embedding)
