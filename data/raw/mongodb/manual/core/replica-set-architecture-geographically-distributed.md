...
If your deployment only runs in two data centers, a single data center outage can prevent your deployment from handling writes if the surviving data center cannot form a majority.

To resolve this issue, you need to reconfigure your replica set by following these instructions:

- For deployments hosted on MongoDB Atlas, see [Reconfigure a Replica Set During a Regional Outage.](https://www.mongodb.com/docs/atlas/reconfigure-replica-set-during-regional-outage/#std-label-reconfigure-rs-during-regional-outage)

- For self-managed deployments, see [Reconfigure Self-Managed Replica Set for Unavailable Members.](/docs/manual/tutorial/reconfigure-replica-set-with-unavailable-members#std-label-reconfigure-replica-set-with-unavailable-members)
...
- Two data centers: two members to Data Center 1 and one member to Data Center 2.
  ...
  - If Data Center 2 goes down, the replica set remains writable as the members in Data Center 1 can hold an election.
...
![Diagram of a 5 member replica set distributed across three data centers.](/images/replica-set-three-data-centers.bakedsvg.svg)

## Electability of Members

Some members of the replica set, such as members that have networking restraint or limited resources, should not be able to become primary in a [failover](/docs/manual/reference/glossary#std-term-failover). Configure members that should not become primary to have [priority 0.](/docs/manual/core/replica-set-priority-0-member#std-label-replica-set-secondary-only-members)

In some cases, you may prefer that the members in one data center be elected primary before the members in the other data centers. You can modify the [`priority`](/docs/manual/reference/replica-configuration#mongodb-rsconf-rsconf.members-n-.priority) of the members such that the members in the one data center has higher [`priority`](/docs/manual/reference/replica-configuration#mongodb-rsconf-rsconf.members-n-.priority) than the members in the other data centers.
...
![Five-member replica set across three data centers with priority 0.5 and priority 0 members.](/images/replica-set-three-data-centers-priority.bakedsvg.svg)
...
- [Deploy a Geographically Redundant Self-Managed Replica Set](/docs/manual/tutorial/deploy-geographically-distributed-replica-set#std-label-server-replica-set-deploy-geo)

- [Deploy a Self-Managed Replica Set](/docs/manual/tutorial/deploy-replica-set#std-label-server-replica-set-deploy)

- [Add an Arbiter to a Self-Managed Replica Set](/docs/manual/tutorial/add-replica-set-arbiter#std-label-server-replica-set-deploy-arbiter)

- [Add Members to a Self-Managed Replica Set](/docs/manual/tutorial/expand-replica-set#std-label-server-replica-set-deploy-expand)
