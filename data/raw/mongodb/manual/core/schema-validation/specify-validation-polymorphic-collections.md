...
You can specify schema validation for a collection that stores [polymorphic data](/docs/manual/data-modeling/design-patterns/polymorphic-data#std-label-polymorphic-data), or documents with varying structures or schemas.
...
1. Create a JSON schema definition for each type of document
...
2. Configure the collection to only accept the appropriate documents

   To allow documents that match either the `customerSchema` or the `accountSchema`, use the `oneOf` JSON schema operator. Then, use the [`collMod`](/docs/manual/reference/command/collMod#mongodb-dbcommand-dbcmd.collMod) command to update the `accounts` collection to use to your schema validation.
...
3. Add extra semantic validations
...
4. Verify the documents in your collection

   To verify that all the documents already in your collection adhere to your new schema validation, use the [`db.collection.validate()`](/docs/manual/reference/method/db.collection.validate#mongodb-method-db.collection.validate) command.
