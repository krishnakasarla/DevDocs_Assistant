...
To modify a collection's schema validation, use the [`collMod`](/docs/manual/reference/command/collMod#mongodb-dbcommand-dbcmd.collMod) command and specify the updated validation in the `validator` object.
...
2. Modify the validation schema.
   ...
   Run the following [`collMod`](/docs/manual/reference/command/collMod#mongodb-dbcommand-dbcmd.collMod) command to change the `minLength` of the `password` field from 8 to 12:
