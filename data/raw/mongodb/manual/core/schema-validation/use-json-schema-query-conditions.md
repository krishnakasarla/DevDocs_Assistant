...
To find documents that either match or don't match a specified schema, use [`$jsonSchema`](/docs/manual/reference/operator/query/jsonSchema#mongodb-query-op.-jsonSchema) with query operators. Similarly, you can update or delete documents based on a schema by using `$jsonSchema` in query conditions for write operations.
...
To find documents in a collection that don't match the schema validation rules, use [`$jsonSchema`](/docs/manual/reference/operator/query/jsonSchema#mongodb-query-op.-jsonSchema) with the [`$nor`](/docs/manual/reference/operator/query/nor#mongodb-query-op.-nor) operator. For example:
...
- [Query Predicates](/docs/manual/reference/mql/query-predicates#std-label-query-projection-operators-top)
