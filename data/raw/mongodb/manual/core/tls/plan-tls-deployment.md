...
TLS (Transport Layer Security) encrypts connections between clients and your MongoDB deployment, and between nodes in a replica set. Before you configure TLS, use this page to decide on the configuration that matches your security requirements.
...
- The type of certificate authority available to you, such as a public or private CA (Certificate Authority).

- Your security requirements, such as whether you need [X.509 authentication.](/docs/manual/core/security-x.509#std-label-security-auth-x509)

The type of CA you have access to determines which TLS configurations are available to you:

| CA Type | Description | Recommended Use |
| --- | --- | --- |
| Public CA | Requires a registered domain name; Does not support `clientAuth` EKU (Extended Key Usage) (as of April 2026) | Intra-cluster TLS encryption without mTLS or X.509 authentication |
| Private CA | Operated internally by your organization's PKI (Public Key Infrastructure); Supports both `serverAuth` and `clientAuth` EKUs | Intra-cluster mTLS and X.509 node authentication |
| Self-signed | You generate the certificate; Does not verify identity as reliably as CA-issued certificates; **Do not use in production** | Local development and testing only |
...
| Authentication Mode | System Requirements |
| --- | --- |
| Keyfile authentication | Shared keyfile on all nodes |
| X.509 node authentication | Intra-cluster mTLS enabled; Client certificates with `clientAuth` EKU issued by a private CA |
...
Cluster member certificates must include X.509 attributes that distinguish them from regular client certificates. To learn about the required certificate attributes, see [Member X.509 Certificate.](/docs/manual/tutorial/configure-x509-member-authentication#std-label-x509-member-certificate)

### Encryption Between Client and Server

When a client such as [`mongosh`](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh) or a driver connects to a TLS-enabled `mongod` instance, the server presents its certificate to prove its identity, and the client verifies the certificate against a trusted CA certificate. The client also confirms that the server's hostname matches the certificate's hostname. If verification succeeds, TLS encrypts the connection between server and client.

Use the following table to determine what you need for each encryption mode:

| Connection Mode | System Requirements |
| --- | --- |
| TLS without client certificate | Set [`allowConnectionsWithoutCertificates`](/docs/manual/reference/configuration-options#mongodb-setting-net.tls.allowConnectionsWithoutCertificates) to `true` in your `mongod`/`mongos` configuration file |
| Mutual TLS or X.509 client authentication | Client must present a certificate with `clientAuth` EKU |

You can set [`allowConnectionsWithoutCertificates`](/docs/manual/reference/configuration-options#mongodb-setting-net.tls.allowConnectionsWithoutCertificates) to `true` to enable the client to connect without sending a certificate. Enabling this prevents users from connecting with incorrect certificates, such as those that only include the `serverAuth` EKU. The server encrypts the connection and proves its own identity to the client, but the server cannot verify the client's identity through the TLS handshake. The client must authenticate through a method like SCRAM.
...
After you establish the TLS connection, clients must authenticate to access the deployment. Clients can authenticate in [many ways](/docs/manual/core/authentication#std-label-authentication), but this quickstart covers SCRAM and X.509 authentication.

Use the following table to determine what you need for each authentication mode:

| Authentication Mode | System Requirements |
| --- | --- |
| SCRAM authentication | Admin account on cluster |
| X.509 client authentication | Mutual TLS between client and server; Certificate with `clientAuth` EKU; Access to private CA |
...
After you determine your configuration using this page, continue to [Obtain TLS Server Certificates.](/docs/manual/core/tls/certificate-tutorial#std-label-tls-certificate-tutorial)
