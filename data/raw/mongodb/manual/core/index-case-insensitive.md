...
Case-insensitive indexes support queries that perform string comparisons without regard for case. Case insensitivity is derived from [collation.](/docs/manual/reference/collation#std-label-collation)

**Important:**

Case-insensitive indexes do not improve performance for [`$regex`](/docs/manual/reference/operator/query/regex#mongodb-query-op.-regex) queries, as the `$regex` operator is not collation-aware and therefore cannot take advantage of such indexes.

## Command Syntax

You can create a case-insensitive index with [`db.collection.createIndex()`](/docs/manual/reference/method/db.collection.createIndex#mongodb-method-db.collection.createIndex) by specifying the `collation` option:
...
| Field | Description |
| --- | --- |
| `locale` | Specifies language rules. For a list of available locales, see [Supported Languages and Locales.](/docs/manual/reference/collation-locales-defaults#std-label-collation-languages-locales) |
| `strength` | Determines comparison rules. A `strength` value of 1 or 2 indicates case-insensitive collation. |

For additional collation fields, see [Collation.](/docs/manual/reference/collation#std-label-collation-document-fields)
