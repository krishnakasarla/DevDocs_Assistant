...
The Crypto Shredding sample app demonstrates how you can make use of MongoDB's [Client-Side Field Level Encryption (CSFLE)](/docs/manual/core/csfle#std-label-manual-csfle-feature) to strengthen procedures for removing sensitive data.
...
You can install and run the application by following the instructions in the [GitHub repository.](https://github.com/mongodb-developer/mongodb-flask-cryptoshredding-example)
...
![A Data Encryption Key store with two users](/images/devcenter_csfle_gdpr_deks.png)
...
![Deleting User B's data encryption key](/images/devcenter_csfle_gdpr_delete_user.png)

## What is CSFLE?

With [CSFLE](/docs/manual/core/csfle#std-label-manual-csfle-feature), applications can encrypt sensitive fields in documents prior to transmitting data to the server. Even when data is being used by the database in memory, it is never in plain text. The database stores and transmits encrypted data that is only deciphered by the client.

CSFLE uses [envelope encryption](/docs/manual/reference/glossary#std-term-envelope-encryption), which is the practice of encrypting plaintext data with a data key, that is itself encrypted by a top level envelope key (also known as a "Customer Master Key", or CMK).

![Envelope encryption diagram](/images/devcenter_csfle_gdpr_dek_diagram.webp)

### Encryption Key Management

CMKs are usually managed by a Key Management Service (KMS). CSFLE [supports multiple KMSs](/docs/manual/core/queryable-encryption/fundamentals/kms-providers#std-label-qe-fundamentals-kms-providers), including Amazon Web Services (AWS), Azure Key Vault, Google Cloud Platform (GCP), and keystores that support the KMIP standard, such as Hashicorp Keyvault. The sample app uses Amazon Web Services as the KMS.

### Automatic and Explicit Encryption

CSFLE can be used in [automatic](/docs/manual/core/csfle/fundamentals/automatic-encryption#std-label-csfle-fundamentals-automatic-encryption) or [explicit](/docs/manual/core/csfle/fundamentals/manual-encryption#std-label-csfle-fundamentals-manual-encryption) mode, or a combination of both. The sample app uses explicit encryption.

- With automatic encryption, you perform encrypted read and write operations based on a defined [encryption schema](/docs/manual/core/csfle/fundamentals/create-schema#std-label-csfle-fundamentals-create-schema), so you don't need application code to specify how to encrypt or decrypt fields.
...
![An example of the crypto shredding UI](/images/devcenter_csfle_gdpr_crypto_shredding_example.png)
...
![A sample UI for adding data](/images/devcenter_csfle_gdpr_data_input_form.png)
...
![Sample data in the UI](/images/devcenter_csfle_gdpr_demo_data_entered.png)
...
![The sample app admin page](/images/devcenter_csfle_gdpr_demo_admin_page.png)
...
![An error message when trying to retrieve encrypted data without a key](/images/devcenter_csfle_gdpr_demo_error_message.png)
...
But what is actually left in the database? You can review the information by returning to the Admin page and clicking Fetch data for all
users. This view doesn't throw an exception if the application can't decrypt the data. Instead, it shows exactly what is stored in the database.
...
![Raw ciphertext from the demo app database](/images/devcenter_csfle_gdpr_raw_ciphertext.png)
...
![mongosh shell output when querying the database](/images/devcenter_csfle_gdpr_mongosh.png)
