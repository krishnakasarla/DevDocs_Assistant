...
- Increased usage of the [WiredTiger](/docs/manual/core/wiredtiger#std-label-storage-wiredtiger) in-memory cache, further improving query speed
...
This model is optimized for [CRUD](/docs/manual/reference/glossary#std-term-CRUD) operations and frequent updates. A bank account balance only needs to reflect the current state, so each account holder's document is updated as that information changes.
...
- `timeField` values that are close together. The time series collection's `granularity`, `bucketMaxSpanSeconds`, and `bucketRoundingSeconds` parameters control the time span covered by each bucket. For more information, see [Set Granularity for Time Series Data.](/docs/manual/core/timeseries/timeseries-granularity#std-label-timeseries-granularity)
...
The bucket catalog is a specialized in-memory cache in [WiredTiger](/docs/manual/core/wiredtiger#std-label-storage-wiredtiger). It tracks buckets to minimize latency and coordinate concurrent writes.
...
To avoid inconsistencies caused by race conditions, buckets may be closed and removed from the bucket catalog when a conflicting operation is executed. Restarting [`mongod`](/docs/manual/reference/program/mongod#mongodb-binary-bin.mongod) closes all buckets and resets the bucket catalog.
...
- MongoDB creates a new bucket if there isn't a suitable one for an incoming document. This occurs when any of the following are true:
  ...
  The starting timestamp of a new bucket is rounded down based on the collection's [granularity](/docs/manual/core/timeseries/timeseries-granularity#std-label-timeseries-granularity). This handles cases where documents with out-of-order timestamps arrive in close succession.

### Closure

MongoDB closes a bucket under any of the following circumstances:

- Time has moved forward or backward past the covered time span, as indicated by an incoming document timestamp that falls outside of the bucket's bounds. These bounds are determined by the collection's [granularity](/docs/manual/core/timeseries/timeseries-granularity#std-label-timeseries-granularity) setting.
...
- The bucket has exceeded its storage size limit. This happens when:
  ...
  - The set of active buckets doesn't fit within the allowed storage engine cache size. You can review this information using the [`collStats`](/docs/manual/reference/command/collStats#mongodb-dbcommand-dbcmd.collStats) database command.
...
- [`mongod`](/docs/manual/reference/program/mongod#mongodb-binary-bin.mongod) restarts. This closes all buckets.

### Deletion

MongoDB deletes a bucket when:

- Its maximum allowed timestamp is less than the current time minus the collection's `expireAfterSeconds` parameter. This is equivalent to a [TTL collection's](/docs/manual/tutorial/expire-data#std-label-ttl-collections) time to live.

- A [`delete`](/docs/manual/reference/command/delete#mongodb-dbcommand-dbcmd.delete) or [`db.collection.deleteMany()`](/docs/manual/reference/method/db.collection.deleteMany#mongodb-method-db.collection.deleteMany) command deletes the last document in the bucket.
